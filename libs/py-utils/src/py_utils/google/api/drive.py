...
class GoogleDriveService(LoggerMixing):
  ...
  def upload_file_to_drive(self, file_path: Path, parent_folder_id: str = None, file_name: str = None) -> str:
    self.logger.info(f"Upload file to drive with file_path: {file_path} and parent_folder_id: {parent_folder_id}")
    file_metadata = {
      "name": file_name if file_name else file_path.name,
      "parents": [parent_folder_id] if parent_folder_id else None
    }
    media = MediaFileUpload(file_path.resolve(),
  ...
  def upload_folder_to_drive(self, folder_path: Path, parent_folder_id: str = None) -> str:
    folder_metadata = {
